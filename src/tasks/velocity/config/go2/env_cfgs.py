...
from src.tasks.velocity.terrains import (
  get_complex_outdoor_terrain_cfg,
  get_stairs_and_holes_terrain_cfg,
)

TerrainType = Literal["flat", "rough", "stairs_holes", "complex"]


def unitree_go2_complex_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create Unitree Go2 complex outdoor terrain velocity configuration with
  stairs, rough terrain, walls, corridors, and pits/holes (Stage 3 - Big Map).
  """
  cfg = unitree_go2_rough_env_cfg(play=play)
  assert cfg.scene.terrain is not None
  cfg.scene.terrain.terrain_generator = get_complex_outdoor_terrain_cfg()
  if play:
...
def unitree_go2_stairs_holes_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create Unitree Go2 intermediate terrain configuration with stairs and holes (Stage 2)."""
  cfg = unitree_go2_rough_env_cfg(play=play)
  assert cfg.scene.terrain is not None
  cfg.scene.terrain.terrain_generator = get_stairs_and_holes_terrain_cfg()
  if play:
    cfg.scene.terrain.terrain_generator.curriculum = False
    cfg.scene.terrain.terrain_generator.num_cols = 4
    cfg.scene.terrain.terrain_generator.num_rows = 4
    cfg.scene.terrain.terrain_generator.border_width = 8.0
  return cfg
...
def unitree_go2_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  # Note: terrain_scan and height_scan are kept active (scanning flat plane)
  # to preserve neural network architecture and dimension compatibility (234 inputs)
  # across all curriculum stages (Flat -> Stairs & Holes -> Complex Big Map).
